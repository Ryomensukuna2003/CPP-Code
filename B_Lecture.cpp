...
string findValueForKey(const vector<pair<string, string>>& vec, const string& key) {
    for (auto& pair : vec) {
        if (pair.first == key) {
            return pair.second; 
        }
...
void solve()
{
    ...
    for(int i=0;i<m;i++){
        ...
        if(s1.size()>s2.size()){
            vec1.push_back(pair<string,string>(s1,s2));
            vec1.push_back(pair<string,string>(s2,s2));
        }
        else{
            vec1.push_back(pair<string,string>(s2,s1));
            vec1.push_back(pair<string,string>(s1,s1));
        }
    }
    for(int i=0;i<n;i++){
        string s;cin>>s;
        string result;
        for (auto& pair : vec1) {
            if (pair.first == s) result=pair.second;
        }
