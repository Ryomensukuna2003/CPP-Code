...
ll calexpression(ll a,ll b,ll c,ll mid){
    ll x=((mid*mid*a)+(mid*b)+(c));
    return x;
}
...
void solve()
{
    ...
    ll l=1,r=ceil(sqrt(x)),mid;
    while(l<=r){
        mid=(l+r)/2;
        ll sum=calexpression(a,b,c,mid);
        ll temp_sum=calexpression(a,b,c,mid-1);
        if(sum>=x && temp_sum<x){
            cout<<mid<<endl;
            return;
        }
